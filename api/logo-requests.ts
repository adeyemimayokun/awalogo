...
import { sendLogoRequestReceipt, sendPrivateLogoRequest } from "./_lib/email.js";
import { createRepositoryIssue } from "./_lib/github.js";
import { methodNotAllowed, requireSameOrigin } from "./_lib/http.js";
import { enrollLogoNotification } from "./_lib/logo-notification-queue.js";
...
export default async function handler(request: VercelRequest, response: VercelResponse): Promise<void> {
  ...
  try {
    ...
    if (submission.notifyWhenAvailable) {
      await enrollLogoNotification(resendAudience, {
        email: submission.email,
        institutionName: submission.institutionName
      });
    }
    await sendPrivateLogoRequest(submission);
    await sendLogoRequestReceipt(submission);
    ...
    response.status(201).json({
      ok: true,
      issue,
      notification: submission.notifyWhenAvailable ? "queued" : "not-requested"
    });
  } catch (error) {
