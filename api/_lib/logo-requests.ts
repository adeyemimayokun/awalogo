...
const optionalHttpUrl = z.union([
  z.literal(""),
  z.string().url().refine((value) => /^https?:/.test(new URL(value).protocol), "Enter a valid website URL")
]);
...
export const publicLogoRequestSchema = z.object({
  ...
  logoFormat: z.enum(["SVG", "PNG", "WebP"]),
...
export function buildPublicIssue(request: PublicLogoRequest): { title: string; body: string } {
  return {
    ...
    body: [
      ...
      "",
      "## Official website",
      request.officialWebsite || "Not provided",
      "",
      "## Submitted logo artwork",
      `- Format: ${request.logoFormat}`,
...
export function buildMaintainerEmail(request: PublicLogoRequest): { subject: string; text: string } {
  return {
    ...
    text: [
      ...
      `Company website: ${request.officialWebsite || "Not provided"}`,
      `Contributor email: ${request.email}`,
      `Logo file type: ${request.logoFormat}`,
